...
def match_images(img1, img2, ratio_thresh=0.75, save_visualization=True):
    ...
    ransac_matches_img = cv2.drawMatches(img1, cv_kp1, img2, cv_kp2, good_matches, None,
                                       flags=cv2.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS)
    if save_visualization:
        cv2.imwrite('matched_features_23.jpg', match_img)
        cv2.imwrite('ransac_matches.jpg_23', ransac_matches_img)
...
if __name__ == "__main__":
    # Read images
    image1_path = r'Phase1/Data/Train/Set1/2.jpg'  # Replace with your image path
    image2_path = r'Phase1/Data/Train/Set1/3.jpg'  # Replace with your image path
    ...
    try:
        ...
        warp = cv2.warpPerspective(img1, results['H_matrix'], (img1.shape[1] + img2.shape[1], img1.shape[0] + img2.shape[0]))


        
        """ # Display results
        cv2.imshow('Feature Matches', results['feature_matches'])
        cv2.waitKey(0)
        cv2.destroyAllWindows()
        ...
        # Display results
        cv2.imshow('RANSAC Matches', results['ransac_matches'])
        cv2.waitKey(0)
        cv2.destroyAllWindows() """

        # Display results
        cv2.imshow('Warped Img', warp)
        cv2.waitKey(0)
        cv2.destroyAllWindows()
