...
def process_patches(image, keypoints, patch_size=41, output_size=8):
    """
    Process patches centered around valid keypoints in an image.
    
    Parameters:
        image (ndarray): Input image (grayscale).
        keypoints (list of tuples): List of (x, y) coordinates of keypoints.
        patch_size (int): Size of the patch (default is 41×41).
        output_size (int): Size after sub-sampling (default is 8×8).
        
    Returns:
        valid_keypoints (list of tuples): Keypoints for which processing was successful.
        feature_vectors (list of ndarray): List of 64×1 standardized feature vectors.
    """
    half_patch = patch_size // 2
    valid_keypoints = []
    feature_vectors = []
    ...
    for keypoint in keypoints:
        ...
        # Check if the patch around the keypoint is fully contained in the image
        if (x - half_patch >= 0 and x + half_patch < image.shape[0] and
            y - half_patch >= 0 and y + half_patch < image.shape[1]):
            
            # Extract the 41×41 patch
            patch = image[x - half_patch:x + half_patch + 1, 
            ...
            # Sub-sample the blurred patch to 8×8
            sub_sampled_patch = cv2.resize(blurred_patch, (output_size, output_size), 
            ...
            mean = np.mean(feature_vector)
            std = np.std(feature_vector)
            standardized_vector = (feature_vector - mean) / (std + 1e-8)
